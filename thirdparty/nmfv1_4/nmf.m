...
switch option.algorithm
    case 'nmfnnls'
        %[A,Y,numIter,tElapsed,finalResidual,~,~,~]=nmfnnlsProgress(X,k,option.optionnmf);
        % DEFAULT and ORIGNAL: 
        [A,Y,numIter,tElapsed,finalResidual]=nmfnnls(X,k,option.optionnmf); % slightly modified to output progress!
        %[A,Y,numIter,tElapsed,finalResidual]=nmfnnlsInit(X,k,option.optionnmf);
    case 'nmfrule'
