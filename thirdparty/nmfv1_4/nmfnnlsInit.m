function [A,Y,numIter,tElapsed,finalResidual]=nmfnnlsInit(X,k,option)
...
%% 
if option.dis && exist( 'AdvancedTextProgressBar', 'class' )
...
for i=1:option.iter
    ...
    % David switched ordering here! ... still seems to work!
    Y=kfcnnls(A,X);
    A=kfcnnls(Y',X');
    A=A';
    A=normc(A);
